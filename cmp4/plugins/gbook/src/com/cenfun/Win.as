package com.cenfun{
	...
	public class Win extends MovieClip {
		...
		//当前微博帐户名，授权后获取
		public static var tqq_selfname:String;
		...
		public var clientip:String = "127.0.0.1";
		public var pageinfo:String = "";
		public var pagetime:String = "0";
		public var totalnum:String = "0";
		
		public var hasnext:String = "0";
		public var gbook_topic:String = "CMP";
		...
		public function formSendClick(e:MouseEvent):void {
			...
			if (form_type == WBHT) {
				...
				reTweet(new TqqApi(TqqApi.re_add), str);
			} else if (form_type == WBHF) {
				reTweet(new TqqApi(TqqApi.reply), str);
			}
		}
		
		public function reTweet(tqqapi:TqqApi, str:String):void {
			tqqapi.params.format = "xml";
			tqqapi.params.clientip = clientip;
			tqqapi.params.content = str;
			tqqapi.params.jing = "";
			tqqapi.params.wei = "";
			tqqapi.params.reid = tweet_id;
		...
		public function tweetDel(id:String):void {
			...
			oauth.load(tqqapi, tweetDelDone);
		}
		...
		public function userTalk(username:String):void {
			tqq_username = username;
			form_type = TALK;
			showForm();
		}

		public function userFollow(username:String):void {
			if (tqq_selfname == username) {
				showMsg("无法收听自己！", 2);
				return;
			}
			tqq_username = username;
		...
		public function showSelfInfo():void {
			...
			if (xmlself) {
				...
				str += '<font size="12px">'
				str += '<a href="'+link+'/follower" target="_blank">听众:<font size="11px">' + Tools.num_format(xmlself.data.fansnum) + '</font></a>  ';
				str += '<a href="'+link+'/mine" target="_blank">广播:<font size="11px">' + Tools.num_format(xmlself.data.tweetnum) + '</font></a>';
				str += '</font>';
		...
		public function showMenu():void {
			var ms:Array = [];
			var wbht:String = "#" + gbook_topic + "#的留言";
			...
			//Create:首页广播（原创）更新数
			if (xmlinfo) {
				var num:String = xmlinfo.data.home.text();
				if ("0" != num) {
					home += "(" + num + "新)";
				}
				num = xmlinfo.data.mentions.text();
				if ("0" != num) {
					ment += "(" + num + "新)";
				}
			}
		...
		public function apiHandler(_api:Object):void {
			...
			if (api.config.gbook_topic) {
				gbook_topic = api.config.gbook_topic;
			}
		...
		public function loadHtTweets():void {
			var tqqapi:TqqApi = new TqqApi(TqqApi.ht_timeline);
			tqqapi.params.format = "xml";
			//分页标识
			if (nexting) {
				//不能往下翻
				...
				tqqapi.params.pageflag = 1;
				tqqapi.params.pageinfo = pageinfo;
			} else {
				tqqapi.params.pageflag = 0;
				tqqapi.params.pageinfo = "";
			}
			//每次请求记录的条数（1-20条）
			tqqapi.params.reqnum = 20;
			//
			tqqapi.params.httext = gbook_topic;
			oauth.load(tqqapi, htTweetsComplete);
		}
		public function htTweetsComplete(ba:ByteArray):void {
			if (!ba) {
				showMsg("无法读取当前话题列表：" + gbook_topic);
				return;
			}
			var xml:XML = oauth.parseXML(ba);
			if (xml.ret == "0") {
				totalnum = xml.data.totalnum.text();
				pageinfo = xml.data.pageinfo.text();
				hasnext = xml.data.hasnext.text();
		...
		public function loadHomeTweets():void {
			...
			if (nexting) {
				...
				tqqapi.params.pageflag = 1;
				tqqapi.params.pagetime = pagetime;
			} else {
		...
		public function selfTweetsComplete(ba:ByteArray):void {
			...
			if (xml.ret == "0") {
				//最后一条记录的时间
				pagetime = "0";
				var infos:XMLList = xml.data.info;
				if (infos) {
					var info:XML = infos[infos.length() - 1];
					if (info) {
						pagetime = info.timestamp.text();
					}
				}
				//
				hasnext = xml.data.hasnext.text();
		...
		public function checkInfoUpdate(type:String = null):void {
			//仅显示时请求
			if (!main.visible) {
				return;
			}
			...
			if (type) {
				...
				tqqapi.params.op = 1;
				if (type == HOME) {
					tqqapi.params.type = 5;
				} else if (type == MENT) {
					tqqapi.params.type = 6;
				}
		...
		public function showForm():void {
			var str:String = "发表广播：";
			if (form_type == WBHT) {
				str = '<b>在#' + gbook_topic + '#发表留言：</b>';
				total = 140 - gbook_topic.length - 3;
			} else if (form_type == TALK) {
				str = '<b>对@' + tqq_username + '说：</b>';
				total = 140 - tqq_username.length - 2;
			} else if (form_type == WBHF) {
				str = '<b>回复：</b><font size="12">(不会出现在你的听众主页上)</font>';
				total = 140;
			} else if (form_type == WBZB) {
				str = '<b>转播：</b><font size="12">(转播原文，并发到给你的听众)</font>';
				total = 140;
			}
